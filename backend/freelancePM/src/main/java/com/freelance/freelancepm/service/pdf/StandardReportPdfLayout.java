...
public class StandardReportPdfLayout implements ReportPdfLayout {
    ...
    // Professional Corporate Blue for Success/Profit metrics
    private static final Color CORPORATE_BLUE = new Color(37, 99, 235);

    @Override
    public void drawHeader(PdfGenerationContext context, Report report, Manager manager, byte[] logoBytes)
            throws IOException {
        ...
        context.drawText("KINGSMAN", margin, textY, gebukFont, 26, Color.WHITE);

        // 3. Right-Aligned Report Designation
        context.drawRightAlignedText("FINANCIAL PERFORMANCE", width - margin, textY, FONT_BOLD, 16, Color.WHITE);

        textY -= 20;
        String dateRange = "Generated on " + report.getGeneratedOn().format(DATE_FORMATTER);
    ...
    @Override
    public void drawSummaryCards(PdfGenerationContext context, Report report) throws IOException {
        PdfStyle style = context.getStyle();
        float margin = context.getMargin();
        float cardWidth = (context.getPageWidth() - (margin * 2) - 20) / 3;
        float y = context.getYPosition();

        drawCard(context, "TOTAL REVENUE", report.getTotalIncome(), margin, y, cardWidth, style.getPrimaryColor());
        drawCard(context, "ESTIMATED EXPENSES", report.getTotalExpense(), margin + cardWidth + 10, y, cardWidth,
                style.getSecondaryColor());
        drawCard(context, "NET PROFIT", report.getTotalProfit(), margin + (cardWidth + 10) * 2, y, cardWidth,
                CORPORATE_BLUE);
    ...
    @Override
    public void drawBreakdownTable(PdfGenerationContext context, List<ReportDetail> details) throws IOException {
        PdfStyle style = context.getStyle();
        float margin = context.getMargin();
        ...
        for (ReportDetail detail : details) {
            ...
            float textY = y + 2;
            context.drawText(detail.getProject().getName(), margin + 10, textY, FONT_BOLD, style.getFontSizeText(),
                    style.getPrimaryColor());
            context.drawText(detail.getClient().getName(), margin + 160, textY, FONT_REGULAR, style.getFontSizeText(),
                    PdfStyle.COLOR_TEXT);

            context.drawRightAlignedText("$" + formatMoney(detail.getIncome()), margin + width - 150, textY,
                    FONT_REGULAR, style.getFontSizeText(), style.getPrimaryColor());
            context.drawRightAlignedText("$" + formatMoney(detail.getExpense()), margin + width - 80, textY,
                    FONT_REGULAR, style.getFontSizeText(), PdfStyle.COLOR_TEXT);
            context.drawRightAlignedText("$" + formatMoney(detail.getProfit()), margin + width - 10, textY, FONT_BOLD,
    ...
    private void drawTableHeader(PdfGenerationContext context) throws IOException {
        ...
        context.drawText("CLIENT", margin + 160, textY, FONT_BOLD, 8, Color.WHITE);
        context.drawRightAlignedText("INCOME", margin + width - 150, textY, FONT_BOLD, 8, Color.WHITE);
        context.drawRightAlignedText("EXPENSE", margin + width - 80, textY, FONT_BOLD, 8, Color.WHITE);
        context.drawRightAlignedText("PROFIT", margin + width - 10, textY, FONT_BOLD, 8, Color.WHITE);
    ...
    public void drawFooter(PdfGenerationContext context, Manager manager) throws IOException {
        ...
        String brand = (manager != null) ? manager.getCompanyName() : "KINGSMAN";
    ...
    private PDFont loadLogoFont(PdfGenerationContext context) throws IOException {
        try (InputStream is = getClass().getResourceAsStream("/fonts/gebuk.ttf")) {
            if (is == null)
                return FONT_BOLD; // Fallback to Helvetica Bold if file missing
