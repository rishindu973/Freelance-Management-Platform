...
public class StandardInvoicePdfLayout implements InvoicePdfLayout {
    ...
    @Override
    public void drawHeader(PdfGenerationContext context, Invoice invoice,
            com.freelance.freelancepm.entity.Manager manager, byte[] logoBytes) throws IOException {
        ...
        if (logoBytes != null) {
            ...
            context.drawText(manager != null ? manager.getCompanyName() : "ANTIGRAVITY", margin + 70, textY, FONT_BOLD,
                    style.getFontSizeTitle(), PdfStyle.COLOR_WHITE);
        } else {
            context.drawText(
                    manager != null && manager.getCompanyName() != null ? manager.getCompanyName() : "FREELANCEFLOW",
                    margin, textY, FONT_BOLD, style.getFontSizeTitle(), PdfStyle.COLOR_WHITE);
        }
        ...
        String billingFrom = manager != null && manager.getAddress() != null ? manager.getAddress()
                : "123 Workspace Dr, Innovation Park";
        context.drawText(billingFrom, margin, textY, FONT_REGULAR, style.getFontSizeSmall(), PdfStyle.COLOR_MUTED);
        context.drawRightAlignedText("Official Document", width - margin, textY, FONT_BOLD, style.getFontSizeSmall(),
    ...
    public void drawClientSection(PdfGenerationContext context, Invoice invoice,
        ...
        if (client.getAddress() != null) {
            context.drawWrappedText(client.getAddress(), margin, y, 200, FONT_REGULAR, style.getFontSizeSmall(),
                    PdfStyle.COLOR_TEXT);
        }
    ...
    public void drawTableHeader(PdfGenerationContext context) throws IOException {
        ...
        context.drawRightAlignedText("QTY", margin + width - 160, textY, FONT_BOLD, style.getFontSizeSmall(),
                PdfStyle.COLOR_MUTED);
        context.drawRightAlignedText("UNIT PRICE", margin + width - 80, textY, FONT_BOLD, style.getFontSizeSmall(),
                PdfStyle.COLOR_MUTED);
    ...
    public void drawTable(PdfGenerationContext context, List<InvoiceLineItem> lineItems) throws IOException {
        ...
        for (InvoiceLineItem item : lineItems) {
            ...
            float textY = y + 2;
            context.drawWrappedText(item.getDescription(), margin + 10, textY, 250, FONT_BOLD, style.getFontSizeText(),
                    style.getPrimaryColor());

            context.drawRightAlignedText(String.valueOf(item.getQuantity()), margin + width - 160, textY, FONT_REGULAR,
            ...
            context.drawRightAlignedText("$" + formatMoney(item.getAmount()), margin + width - 10, textY, FONT_BOLD,
    ...
    public void drawTotalSection(PdfGenerationContext context, Invoice invoice) throws IOException {
        ...
        context.drawRightAlignedText("$" + formatMoney(invoice.getTax()), valueX, y, FONT_BOLD, style.getFontSizeText(),
    ...
    public void drawFooter(PdfGenerationContext context, com.freelance.freelancepm.entity.Manager manager)
        ...
        context.drawText("Terms: Payment due within 30 days. Late fees may apply.", margin, y - 12, FONT_REGULAR, 6,
                PdfStyle.COLOR_MUTED);
    }
